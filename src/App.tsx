...
export default function App() {
  ...
  useEffect(() => {
    ...
    setDebts(prevDebts => {
      ...
      cards.forEach(card => {
        ...
        const existingIndex = updated.findIndex(d => d.creditor === creditorName);
        if (existingIndex === -1) {
          ...
          // Se o valor fixo do cartão mudou depois (ex: usuária ainda editando o campo) e não há pagamento registrado, sincroniza
          const existingDebt = updated[existingIndex];
          const noPayments = !existingDebt.payments || existingDebt.payments.length === 0;
          if (existingDebt.status === 'pending' && noPayments && existingDebt.totalAmount !== card.fixedInvoiceAmount) {
  ...
  // Sugere um valor de juros com base em quantos dias o pagamento ficou atrasado em relação ao vencimento.
  // Fórmula simples: 2% de multa fixa + 0,033% ao dia de juros de mora (aprox. 1% ao mês) sobre o valor pago.
  const calcSuggestedInterest = (dueDate: string, paymentDate: string, amount: number) => {
    const due = new Date(dueDate + 'T00:00:00');
    const paid = new Date(paymentDate + 'T00:00:00');
    const diffDays = Math.round((paid.getTime() - due.getTime()) / (1000 * 60 * 60 * 24));
    if (diffDays <= 0 || amount <= 0) return 0;
    const multa = amount * 0.02;
    const jurosMora = amount * 0.00033 * diffDays;
    return Number((multa + jurosMora).toFixed(2));
  };

  const registerPayment = (debtId: string, amount: number, interest: number = 0, paymentDate?: string) => {
  ...
  return (
    <div className="min-h-screen bg-[#EFF5FF] text-[#1E293B] font-sans pb-32">
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-5">
        {/* Cartão "Hero" estilo app bancário */}
        <div className="relative overflow-hidden rounded-[28px] bg-gradient-to-br from-[#2563EB] via-[#3B82F6] to-[#22C55E] p-5 sm:p-6 mb-5 shadow-lg shadow-blue-900/10">
          <div className="absolute -right-10 -top-14 w-44 h-44 rounded-full bg-white/10"></div>
          <div className="absolute -right-2 bottom-[-40px] w-28 h-28 rounded-full bg-white/10"></div>

          <div className="relative flex items-start justify-between gap-3">
            <div>
              <p className="text-white/80 text-xs font-bold">Olá, Curtis 👋</p>
              <p className="text-white text-[15px] font-black mt-0.5">Suas finanças em um só lugar</p>
            </div>
            <div className="flex items-center gap-1.5 px-2 py-1.5 bg-white/15 backdrop-blur-sm rounded-2xl border border-white/20 shrink-0">
              <button 
              ...
              <div className="w-[1px] h-3.5 bg-white/30"></div>
              <label className="p-1.5 text-white/90 hover:text-white transition-colors cursor-pointer" title="Importar Backup">
          ...
          <div className="relative mt-5">
            <p className="text-white/75 text-[10px] font-black uppercase tracking-wider">Total devido no mês</p>
            <p className="text-white text-4xl font-black tracking-tight mt-1">{formatCurrency(metrics.totalDebt)}</p>
          </div>

          <button 
            onClick={() => setShowCardManager(!showCardManager)}
            className="relative mt-5 w-full sm:w-auto px-5 py-2.5 bg-white text-[#2563EB] rounded-2xl text-xs font-black uppercase tracking-wide transition-all shadow-sm flex items-center justify-center gap-2 hover:bg-blue-50 active:scale-[0.98]"
            title="Gerenciar Cartões"
          >
            <CreditCardIcon className="w-4 h-4" /> Ver meus cartões
          </button>
        </div>

        {/* Sub-tabs segment control for Início sub-tabs */}
        {['dashboard', 'transactions', 'cards', 'negotiations'].includes(activeTab) && (
          <div className="flex gap-1 overflow-x-auto no-scrollbar bg-white p-1.5 rounded-2xl mb-6 shadow-sm border border-[#E2E8F0] shrink-0">
            <button
              onClick={() => setActiveTab('dashboard')}
              className={`flex-1 py-2.5 px-3 text-xs font-bold rounded-xl whitespace-nowrap transition-all flex items-center justify-center ${activeTab === 'dashboard' ? 'bg-[#2563EB] text-white shadow-sm font-black' : 'text-[#64748B] hover:text-[#1E293B]'}`}
            >
              Visão Geral
            </button>
            <button
              onClick={() => setActiveTab('transactions')}
              className={`flex-1 py-2.5 px-3 text-xs font-bold rounded-xl whitespace-nowrap transition-all flex items-center justify-center ${activeTab === 'transactions' ? 'bg-[#2563EB] text-white shadow-sm font-black' : 'text-[#64748B] hover:text-[#1E293B]'}`}
            >
              Extrato
            </button>
            <button
              onClick={() => setActiveTab('cards')}
              className={`flex-1 py-2.5 px-3 text-xs font-bold rounded-xl whitespace-nowrap transition-all flex items-center justify-center ${activeTab === 'cards' ? 'bg-[#2563EB] text-white shadow-sm font-black' : 'text-[#64748B] hover:text-[#1E293B]'}`}
            >
              Cartões
            </button>
            <button
              onClick={() => setActiveTab('negotiations')}
              className={`flex-1 py-2.5 px-3 text-xs font-bold rounded-xl whitespace-nowrap transition-all flex items-center justify-center ${activeTab === 'negotiations' ? 'bg-[#2563EB] text-white shadow-sm font-black' : 'text-[#64748B] hover:text-[#1E293B]'}`}
            >
              Espera / Acordos
            </button>
          </div>
          ...
          {activeTab === 'dashboard' && (
            ...
            >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
                <div className="bg-white p-4 md:p-5 rounded-2xl border border-[#DCFCE7] shadow-sm relative overflow-hidden">
                  <div className="absolute -right-4 -top-4 w-16 h-16 rounded-full bg-[#DCFCE7]/60"></div>
                  <p className="relative text-[9px] md:text-[11px] font-bold text-[#64748B] uppercase tracking-wider mb-1 md:mb-2 text-center md:text-left">Limite Disponível</p>
                  <p className={`relative text-xl md:text-3xl font-bold text-center md:text-left ${metrics.totalAvailable > 0 ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>
                    {formatCurrency(metrics.totalAvailable)}
                  </p>
                </div>
                <div className="bg-white p-4 md:p-5 rounded-2xl border border-[#FEE2E2] shadow-sm relative overflow-hidden">
                  <div className="absolute -right-4 -top-4 w-16 h-16 rounded-full bg-[#FEE2E2]/60"></div>
                  <p className="relative text-[9px] md:text-[11px] font-bold text-[#64748B] uppercase tracking-wider mb-1 md:mb-2 text-center md:text-left">Dívidas (Mês)</p>
                  <p className="relative text-xl md:text-3xl font-bold text-[#DC2626] text-center md:text-left">
                    {formatCurrency(metrics.totalDebt)}
                  </p>
                </div>
                <div className="bg-white p-4 md:p-5 rounded-2xl border border-[#E2E8F0] shadow-sm flex flex-col justify-center gap-2 col-span-2 md:col-span-1">
                  <div className="grid grid-cols-2 md:grid-cols-1 gap-2">
                    <button 
                      onClick={() => { setQuickAddTab('transaction'); setShowQuickAdd(true); }}
                      className="w-full bg-[#2563EB] text-white py-2.5 rounded-xl text-[9px] md:text-[10px] font-bold uppercase tracking-wide hover:bg-[#1D4ED8] active:scale-[0.98] transition-all"
                    >
                      + Novo Gasto
                    </button>
                    <button 
                      onClick={() => { setQuickAddTab('debt'); setShowQuickAdd(true); }}
                      className="w-full bg-[#1E293B] text-white py-2.5 rounded-xl text-[9px] md:text-[10px] font-bold uppercase tracking-wide hover:bg-black active:scale-[0.98] transition-all"
                    >
                      + Nova Dívida
                    </button>
                  </div>
                </div>
                <div className="bg-white p-4 md:p-5 rounded-2xl border border-[#DBEAFE] shadow-sm col-span-2 md:col-span-1 relative overflow-hidden">
                  <div className="absolute -right-4 -top-4 w-16 h-16 rounded-full bg-[#DBEAFE]/60"></div>
                  <p className="relative text-[9px] md:text-[11px] font-bold text-[#64748B] uppercase tracking-wider mb-1 md:mb-2 text-center md:text-left">Total da Obra (Mês)</p>
                  <p className="relative text-xl md:text-3xl font-bold text-[#2563EB] text-center md:text-left">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="md:col-span-8 bg-white rounded-2xl border border-[#E2E8F0] shadow-sm flex flex-col overflow-hidden">
                            ...
                            <td className="px-5 py-3.5">
                              <div className="flex flex-col gap-2" onClick={(e) => e.stopPropagation()}>
                                <div className="flex items-center gap-2">
                                  ...
                                  <div className="flex flex-col">
                                    <label className="text-[8px] font-bold text-[#94A3B8] uppercase tracking-tighter">Data Pgto.</label>
                                    <input
                                      id={`desktop-pay-date-${debt.id}`}
                                      type="date"
                                      defaultValue={new Date().toISOString().split('T')[0]}
                                      className="w-28 text-xs font-bold text-[#1E293B] bg-[#F1F5F9] border border-[#E2E8F0] p-1.5 rounded outline-none"
                                    />
                                  </div>
                                  <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button
                                        type="button"
                                        title="Calcular juros com base no atraso"
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          const dateInput = document.getElementById(`desktop-pay-date-${debt.id}`) as HTMLInputElement;
                                          const valInput = document.getElementById(`desktop-pay-input-${debt.id}`) as HTMLInputElement;
                                        ...
                                        className="text-[9px] font-bold text-[#2563EB] bg-[#EFF6FF] px-1.5 py-1.5 rounded hover:bg-[#DBEAFE]"
                                      >
                                        Sugerir
                                      </button>
                                    </div>
                        ...
                        <div key={debt.id} className="p-4 space-y-3">
                          ...
                          <div className="flex flex-col gap-2 w-full">
                            <div className="flex items-center gap-2 bg-[#F8FAFC] p-2 rounded-2xl border border-[#E2E8F0] w-full">
                                ...
                                <div className="flex flex-col flex-1">
                                  <label className="text-[8px] font-black text-[#94A3B8] uppercase px-1">Data Pgto.</label>
                                  <input
                                    id={`dashboard-pay-date-${debt.id}`}
                                    type="date"
                                    defaultValue={new Date().toISOString().split('T')[0]}
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const dateInput = document.getElementById(`dashboard-pay-date-${debt.id}`) as HTMLInputElement;
                                const valInput = document.getElementById(`dashboard-pay-input-${debt.id}`) as HTMLInputElement;
                                const interestInput = document.getElementById(`dashboard-pay-interest-${debt.id}`) as HTMLInputElement;
                                const suggestion = calcSuggestedInterest(debt.dueDate, dateInput.value, Number(valInput.value));
                                interestInput.value = String(suggestion);
                              }}
                              className="w-full bg-[#EFF6FF] text-[#2563EB] py-1.5 rounded-xl text-[10px] font-bold"
                            >
                              Sugerir juros pelo atraso
                            </button>
                            <button
                              onClick={() => {
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        const valInput = document.getElementById(`manage-pay-input-${debt.id}`) as HTMLInputElement;
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#E2E8F0] shadow-2xl py-2 px-4 flex justify-around items-center rounded-t-3xl">
        <div className="w-full max-w-xl mx-auto flex justify-around items-center">
          <button
            onClick={() => setActiveTab('dashboard')}
            className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-2xl text-center flex-1 transition-all cursor-pointer ${
              ['dashboard', 'transactions', 'cards', 'negotiations'].includes(activeTab)
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-2xl text-center flex-1 transition-all cursor-pointer ${
              activeTab === 'debts' ? 'text-[#2563EB] bg-[#EFF6FF]' : 'text-[#64748B] hover:text-[#1E293B]'
            }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 py-1.5 px-4 rounded-2xl text-center flex-1 transition-all cursor-pointer ${
              activeTab === 'work-summary' ? 'text-[#2563EB] bg-[#EFF6FF]' : 'text-[#64748B] hover:text-[#1E293B]'
            }`}
          >
